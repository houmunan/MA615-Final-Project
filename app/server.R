...
shinyServer(function(input, output) {
  ...
  output$Plottttt <- renderPlot({
    ...
    plot(map.data$long[map.data$group==1],map.data$lat[map.data$group==1], type="l", xlim = c(-125,-66), ylim = c(25,50), col = "grey", xlab = "Longitude", ylab = "Latitude")
    for (i in c(2:63)) {
      points(map.data$long[map.data$group==i],map.data$lat[map.data$group==i], type="l", col = "grey")
    }
