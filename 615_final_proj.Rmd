...
#Description
This project uses package:twitteR and package:streamR to analyse word frequencies of a mass amount of Twitter, compare them among different locations, and is trying to find some underlying relations of the data.

#Parts
I would like to generate both time and location based results.

1. Data Grabbing and Cleaning
2. Geolocation Analysis
3. Popularity Analysis
4. Text mining
5. Conclusion

#Detail of the main part of this paper
1. Data Grabbing and Cleaning

Use package:streamR to stream 29 sample twitter dataset, each sample includes 5 minutes' twitter within an specific hour (from the last hour of 30 Nov to the fourth hour of 1 Dec (Est)).

Cleaning includes: Merge all the subdatasets and add time_mark (for further adjustment of time-zone error, and geolocation analysis), clean the datasets, filter out sub-datasets for five states (MA, NY, DC, IL, CA).

2. Geolocation Analysis:

Generate an interactive distribution plot of all twitters, using ShinyApp, to showed the trend of all twitter's distribution within the continent of United States through timeline, within about one day(and a little more at both tails), separated by the intervals of one hour. I was planned to use ggvis here until I realized that it is based on ShinyApp, and what I want to do is only available in the ShinyApp.

The app is in another project's folder within this repository named "app".


3. Popularity Analysis: Test their differences among five different states (MA, NY, DC, IL, CA).


4. Text mining: 

Generate a wordcloud, which means, to generate a proper corpus first.
...
Here I grabbed 29 datasets, each contains 300 seconds grabbing of tweets. The filter are the region within United States, "(-125, 25, -66, 50)". As long as continuing grab 24 hours' twitter will definitely overflow my RAM and crash my computer, so my strategy is: Capture a 300 seconds (5 minutes) sample for each hour of the day. The code is like this:


...and the datasets are under the location of this project.

#2. Data cleaning and tidying.
I imported all the datasets into R, and marked them with different labels of their belonging hours, such as, 0, 1, 2, 3, ... , 28, each represents 23:00 ~ 23:59 of 29 Nov, 00:00 ~ 00:59 of 30 Nov, etc.
```{r, warning=FALSE}
...
```
I can't fix the warning messages, but this part just works fine to me.

Then, add "time_mark" to each of the dataframes.
```{r}
...
Then, keep only needed variables:
```{r}
...
And, as long as tweets.df is too large while generating word cloud, I choose a subset. the sample for the wordcloud part:


Remove tweets with no geo_enabled and not English. Also filter tweets with extreme locations.
```{r}
...
After that, prepare the five-state sub-datasets, for further popularity analysis:
```{r}
...
```
As is shown in the cluster Dendrogram, 'can', 'latest', 'careerarc', 'hiring', 'job' are in one cluster, which really makes sense that they are all related to jobs. But other clusters are dfficult to interpret and the word 'eduaubdedubu' appears again. We should look back to the text data cleaning and find out how we can make sure every word in my output is meaningful.
